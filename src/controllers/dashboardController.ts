...
export class DashboardController {
  ...
  public async getMostBoughtProductsOverTime(req: Request, res: Response) {
    ...
    try {
      const products = await this.dashboardService.getMostBoughtProductsOverTime(startTime, endTime);
  ...
  public async getProductsNotBought(req: Request, res: Response) {
    ...
    const endTime = req.query.endTime as unknown as Date;
    try {
      const products = await this.dashboardService.getProductsNotBought(startTime, endTime);
      if (products.length === 0) {
        return res.status(404).json({ error: 'No Products found' })
      }
      return res.status(200).json(products)
