...
import { InvalidCredentialsError, UserAlreadyExistsError } from '../Errors/AuthenticationErrors'
...
describe('AuthService', () => {
  ...
  describe('login', () => {
    ...
    it('should throw InternalServerError if JWT secret is not defined P1', async () => {
      ...
      await expect(authService.login(email, password)).rejects.toThrow(InternalServerError)
    })
    ...
    it('should log error and throw InternalServerError on unexpected errors P1', async () => {
      ...
      ;(userRepository.findByEmail as jest.Mock).mockRejectedValue(new Error('Unexpected error'))
  ...
  describe('register', () => {
    it('should register a new user and return the user P0', async () => {
      ...
      const password = 'password123'
      const hashedPassword = await bcrypt.hash(password, 10)
      const newUser = { id: 1, name, email, password: hashedPassword, address: 'put address here', role: 'user' } as User
    ...
    it('should throw UserAlreadyExistsError if the user already exists P1', async () => {
      ...
      await expect(authService.register(name, email, password)).rejects.toThrow(UserAlreadyExistsError)
    })
    ...
    it('should log error and throw InternalServerError on unexpected errors P1', async () => {
      ...
      await expect(authService.register(name, email, password)).rejects.toThrow(InternalServerError)
      expect(mockLogger.error).toHaveBeenCalled()
    })
  })
  ...
  describe('logout', () => {
    ...
    it('should log error and throw InternalServerError on unexpected errors P1', async () => {
      ...
      ;(isTokenBlacklisted as jest.Mock).mockImplementation(() => { throw new Error('Unexpected error') })

      await expect(authService.logout(token)).rejects.toThrow(InternalServerError)
      expect(mockLogger.error).toHaveBeenCalled()
    })
  })
})
